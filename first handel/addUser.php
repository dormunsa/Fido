<?php
    ...
    $query1 = "SELECT * FROM tb_data_215 order by post_id";    
    ...
    if(isset($_POST['dateTime'])){
        ...
        $query2 = "INSERT into fido_users(id,fullName,location,Relation Status,Children,HomeType,Outdoor,Availability,Expirience,Partners,Energetic,Fur,Maturity,Friendly,Obidience,Danagrus,Priority) values ('1', '$_fullNAme','$_location','$_relationStat','$_children','$_homeType','$_outdoor','$_availble','$_expirience','$_partners','$_energetic','$_fur','$_maturity','$_friendly','$_obidience',$_danagrus,$_priority)";
        ...
        //GET: get data again
        $query2 = "SELECT * FROM tb_data_215 order by post_id";
        $result = mysqli_query($connection, $query2);
    }
    ...
    while($row = mysqli_fetch_assoc($result)){ //result are in an associative array. keys are cols names 
        echo "<li><h2>" . $row["description"] . "</h2><h6>" . $row["date"] . "</h6></li>";
    }
    echo "</ul>";
